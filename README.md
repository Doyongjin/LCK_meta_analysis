...
| F. 밴 내성 지수 | 챔프폭·주력 의존도·밴 시 승률 하락을 LCK 분포 percentile 기반으로 종합한 0~100 점수 |
| G. 팀 색깔 프로파일 | 팀을 시스템형·캐리 의존형·초반 압박형·후반 역전형으로 분류, 두 팀 레이더 차트 비교 |
| H. 스페셜리스트 챔피언 | LCK 동라인 평균 대비 유독 잘하는 챔피언 식별 + 조커픽(LCK 점유율 70%+) 탐지, 팀 비교 모드 |
...
- **분석 반영**: D(저격 밴) 게임 번호별 분리 집계, F(밴 내성) 앞 경기 픽 소진 보정, H(스페셜리스트) 강제 픽 비율 표시
